pipeline {
  ...
  environment {
    ...
    awsecrRegistry = '814835082161.dkr.ecr.ap-northeast-2.amazonaws.com/eks-demo-repo'
    ...
    stage('Deploy') { 
      steps {
        ...
        git credentialsId: githubCredential,
            url: 'https://github.com/leejuwan0079/kustomize.git',
            branch: 'main'  
    ...
    stage('Push to Git Repository') {
      steps {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: githubCredential, usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD']]) {
             sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/leejuwan0079/kustomize.git"       
        }
      }
    }
  }
